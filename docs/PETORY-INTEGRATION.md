...
| 엔드포인트 | 설명 | 인증 |
|-----------|------|------|
| `GET /popular/{context}` | 업종별 인기 상호 목록 (최대 20개) | `X-API-Key` (일반) |
...
`PetDataApiClient.buildClient()` 에서 `defaultHeader` 로 고정 설정하면 됩니다:
...
`X-Request-Id`는 이미 `withOptionalCorrelation()` 로 요청별로 주입 중 → **변경 불필요**.
...
```bash
# Petory 요청만
grep 'caller=petory' app.log

# 특정 request_id 전체 추적
grep 'a1b2c3d4' app.log

# 인증 실패 전체
grep 'auth 40' app.log

# popular 503 (캐시 비어있음)
grep 'status=503' app.log | grep 'op=popular'

# 배치 수집 결과
grep 'ingestion-digest' app.log
```
...
| `401 Invalid API Key` | 평문 키 오류 | .env 평문 키 확인 (해시값을 헤더에 넣으면 이 메시지) |
| `403 Admin key required` | 일반 키로 관리자 경로 호출 | `/collect/trigger`는 관리자 키만 |
| `404 Unknown context/category` | 잘못된 경로 파라미터 | 유효값 목록(§1) 참조 |
| `503 popular data unavailable` | Redis 키 없음 (배치 미실행) | `/collect/trigger` 로 배치 실행 또는 빈 상태로 fallback |
| `503 Cache unavailable` | Redis 연결 오류 | `/readyz` 로 Redis 상태 확인 |

`popular` 503 수신 시 Petory 권장 처리: 인기 카드 숨김, 빈 배열 반환, 재시도 없이 다음 배치 대기.
...
| 증상 | 원인 | 상태 |
|------|------|------|
| `in_cache=3` (popular/cafe 항목 수 적음) | 카페 특성상 상호명 패턴이 블로그보다 다양 | 배치 재실행으로 개선 예정 |
| `'진짜'` 같은 부사가 상호명으로 포함 | blocklist 미등록 | 수정 예정 |
